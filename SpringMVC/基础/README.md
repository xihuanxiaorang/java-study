...
## 1.2 2、SpringMvc 核心配置文件 - dispatcher. Xml
...
![](attachments/Pasted%20image%2020220915044648.png)
...
![](attachments/Pasted%20image%2020220915044721.png)

## 1.4 4、配置 Tomcat

TODO...

# 2 2、第一个 SpringMvc 程序
...
### 2.2.1 1、一种类型的 SpringMvc 控制器被创建几次？
...
![](attachments/Pasted%20image%2020220915044755.png)

# 3 3、SpringMvc 控制器开发详解 (一)
...
### 3.2.1 1、基于 Servlet API 接收客户端请求参数
...
![](attachments/Pasted%20image%2020220915044820.png)
...
### 3.4.2 2、回顾 JavaWeb 开发中中文字符集乱码的解决方案
...
![](attachments/Pasted%20image%2020220915044902.png)  
那么我们该怎么解决这个问题呢🤔？
...
![](attachments/Pasted%20image%2020220915044919.png)  
...
![](attachments/Pasted%20image%2020220915044935.png)

### 3.6.2 2、接收 Cookie 数据
...
![|700](attachments/Pasted%20image%2020220915044955.png)  
![](attachments/Pasted%20image%2020220915045011.png)
...
![](attachments/Pasted%20image%2020220915045023.png)

# 4 4、SpringMvc 控制器开发详解 (二)
...
## 4.2 2、SpringMvc 控制器调用业务对象 (SSM 整合)

### 4.2.1 1、具体步骤

#### 4.2.1.1 1、新建一个子模块 spring-mvc-study-02
...
#### 4.2.1.2 2、新建一个 tomcat 启动器 ssm

![|800](attachments/Pasted%20image%2020220915045039.png)  
![|800](attachments/Pasted%20image%2020220915045059.png)

#### 4.2.1.3 3、在 dispatcher.xml 增加 Mybatis 与事务需要用到的核心 Bean
...
#### 4.2.1.4 4、新建 User 实体类，省略 getter、setter
...
#### 4.2.1.6 6、新建 UserMapper 接口和 UserMapper.xml
...
#### 4.2.1.7 7、新建 UserService 接口以及实现类 UserServiceImpl
...
#### 4.2.1.8 8、新建 UserController 控制器类
...
![](attachments/Pasted%20image%2020220915045123.png)  
或者查看 Controller 中注入的 Service 是否代理对象也可以辨别到底是否开启事务，断点查看之后，注入进来的 Service 并不是代理对象，所以说明并没有开启事务功能！  
![](attachments/Pasted%20image%2020220915045129.png)  
这是为什么呢🤔？
...
![](attachments/Pasted%20image%2020220915045145.png)

# 5 5、SpringMvc 控制器开发详解 (三)
...
## 5.2 2、JavaWeb 开发中流程跳转的回顾

### 5.2.1 1、JavaWeb 开发中流程跳转的核心代码
...
![](attachments/Pasted%20image%2020220915045207.png)  
如果使用 "forward: result"; 则会抛出 404 异常！  
![](attachments/Pasted%20image%2020220915045214.png)

### 5.3.2 2、控制器 redirect 页面
...
![](attachments/Pasted%20image%2020220915045224.png)  
如果使用 "redirect: result"；则会抛出 404 异常！  
![](attachments/Pasted%20image%2020220915045230.png)

### 5.3.3 3、控制器 forward 控制器
...
![](attachments/Pasted%20image%2020220915045240.png)

### 5.3.4 4、控制器 redirect 控制器
...
![](attachments/Pasted%20image%2020220915045246.png)

## 5.4 4、Web 开发中对于作用域的处理

### 5.4.1 1、回顾 JavaWeb 开发中三种作用域及其使用场景
...
### 5.4.2 2、SpringMvc 中对于作用域的处理

#### 5.4.2.1 1、SpringMvc 中对 request 作用域的处理
...
![](attachments/Pasted%20image%2020220915045254.png)
...
![](attachments/Pasted%20image%2020220915045302.png)
...
#### 5.4.2.2 2、SpringMvc 中对 session 作用域的处理
...
![](attachments/Pasted%20image%2020220915045308.png)  
...
![|600](attachments/Pasted%20image%2020220915045319.png)  
控制台打印：  
![](attachments/Pasted%20image%2020220915045338.png)  
说明 session 作用域中的数据真的被清除了！

#### 5.4.2.3 3、SpringMvc 中对 application 作用域的处理
...
![](attachments/Pasted%20image%2020220915045351.png)  
...
![|700](attachments/Pasted%20image%2020220915045402.png)  
控制台打印：  
![](attachments/Pasted%20image%2020220915045413.png)  
...
![](attachments/Pasted%20image%2020220915045418.png)  
如需要将请求参数的数据存放在 session 作用域当中，则只能使用传统的做法 (Model 或者 ModelMap)。

### 5.4.4 4、ModelAndView 技术 (了解)
...
![](attachments/Pasted%20image%2020220915045425.png)
...
![|500](attachments/Pasted%20image%2020220915045434.png)  
控制台打印：  
![](attachments/Pasted%20image%2020220915045452.png)

### 5.5.3 3、视图控制器的 redirect 跳转
...
![](attachments/Pasted%20image%2020220915045501.png)
...
#### 5.6.3.2 2、default-servlet-handler
...
![](attachments/Pasted%20image%2020220915045512.png)

# 6 6、遇到的问题💣

## 6.1 1、Web 版本与 Tomcat 版本不匹配

```ad-bug
org.apache.tomcat.util.descriptor.web.WebXml.setVersion 未知版本字符串 [4.0]。将使用默认版本。
```

![](attachments/Pasted%20image%2020220915045527.png)
...
![](attachments/Pasted%20image%2020220915045534.png)

## 6.2 2、找到多个名为 spring_web 的片段。这是不合法的相对排序

```ad-solution
将 target 目录删除后，重新编译运行。
```

## 6.3 3、Tomcat 启动的时候，控制台输出乱码
...
![|800](attachments/Pasted%20image%2020220915045544.png)  
![|800](attachments/Pasted%20image%2020220915045559.png)
