...
const taskInfo = (identifier) => {
  return http.get(`/oss/tasks/${identifier}`);
};
...
const preSignUrl = ({ identifier, partNumber }) => {
  return http.get(`/oss/tasks/${identifier}/${partNumber}`);
};
...
const merge = (identifier) => {
  return http.post(`/oss/tasks/merge/${identifier}`);
};

export { taskInfo, initTask, preSignUrl, merge };
